...
export default function UiPrototype() {
  ...
  const handleAddTask = (e) => {
    if (e.key === 'Enter' && inputValue.trim() !== '') {
      setCustomTasks([
        ...customTasks,
        { id: Date.now(), title: inputValue.trim(), time: formatTime(new Date()) },
      ])
      setInputValue('')
      setIsAdding(false)
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-800 via-slate-900 to-black text-white font-sans flex items-center justify-between p-8 relative overflow-hidden">
      ...
      <div className="flex flex-col gap-6 z-10 max-w-md">
        ...
        <div className="p-6 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10">
          <p className="text-sm text-slate-300 mb-4">
            点击下方按钮，观察右侧列表中 "Robocon 紧急会议" 的
            <strong className="text-orange-400 font-normal">微光渗漏</strong>效果。
          </p>
          <button
      ...
      <div className="w-80 h-[85vh] bg-white/[0.03] backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl flex flex-col overflow-hidden relative z-10">
        <div className="px-6 pt-8 pb-4 border-b border-white/5">
          <div className="text-4xl font-extralight tracking-tight">{formatTime(time)}</div>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
          ...
          {customTasks.map((task) => (
            <div
              key={task.id}
              className="flex gap-4 p-3 rounded-xl bg-white/5 border border-white/5 transition-all"
            >
              <div className="w-12 text-right text-sm font-mono mt-0.5 text-slate-400">
                {task.time}
              </div>
              <div className="flex-1">
          ...
          <div
            className={`flex gap-4 p-3 rounded-xl transition-all duration-1000 ease-in-out relative ${
              isUrgent
          ...
          >
            <div
              className={`absolute inset-0 rounded-xl transition-opacity duration-1000 shadow-[inset_4px_0_0_0_rgba(249,115,22,0.8),0_0_30px_rgba(249,115,22,0.15)] ${
                isUrgent ? 'opacity-100 animate-pulse' : 'opacity-0'
              }`}
            ></div>
            <div
              className={`w-12 text-right text-sm font-mono mt-0.5 transition-colors duration-500 ${
            ...
            <div className="flex-1 relative z-10">
              <div
                className={`text-sm font-medium transition-colors duration-500 ${
                  isUrgent ? 'text-orange-50' : 'text-slate-300'
                }`}
              >
                Robocon 紧急方案评审
              </div>
              <div
                className={`text-xs mt-1 flex items-center gap-1 transition-colors duration-500 ${
                  isUrgent ? 'text-orange-300' : 'text-slate-500'
                }`}
              >
